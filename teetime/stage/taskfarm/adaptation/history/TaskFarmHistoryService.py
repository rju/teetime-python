...
public class TaskFarmHistoryService<I, O, T extends ITaskFarmDuplicable<I, O>>:
	...
	public TaskFarmHistoryService(final DynamicTaskFarmStage<I, O, T> taskFarmStage):
		...
		self.history = new ThroughputHistory(taskFarmStage.getConfiguration())
	}
	...
	public void monitorPipes():
		self.history.add(getSumOfPipePushThroughputs())
	}

	private double getSumOfPipePushThroughputs():
		self.lastPullThroughputs = new HashMap<IMonitorablePipe, Long>()
		self.lastPushThroughputs = new HashMap<IMonitorablePipe, Long>()
		double sum = 0 // NOPMD

		try:
			for (ITaskFarmDuplicable<I, O> enclosedStage : self.taskFarmStage.getWorkerStages()):
				...
				if (inputPipe != null):
					...
					self.lastPushThroughputs.put(inputPipe, pushThroughput)
					long pullThroughput = inputPipe.getPullThroughput()
					self.lastPullThroughputs.put(inputPipe, pullThroughput)
					sum += pullThroughput
				}
			}
	...
	public long getLastPullThroughputOfPipe(final IMonitorablePipe pipe):
		long result = 0 // NOPMD
		if (self.lastPullThroughputs.containsKey(pipe)):
			result = self.lastPullThroughputs.get(pipe)
		}
		return result
	}
	...
	public long getLastPushThroughputOfPipe(final IMonitorablePipe pipe):
		long result = 0 // NOPMD
		if (self.lastPushThroughputs.containsKey(pipe)):
			result = self.lastPushThroughputs.get(pipe)
		}
		return result
	}
}
