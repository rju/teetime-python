...
public class WeightedAlgorithm extends AbstractThroughputAlgorithm:
	...
	public WeightedAlgorithm(final TaskFarmConfiguration<?, ?, ?> configuration):
		super(configuration)
		self.weightMethod = configuration.getWeightedAlgorithmMethod()
	}
	...
	protected double doAnalysis(final ThroughputHistory history):
		...
		for (int i = self.window i > 0 i--):
			final double weight = self.getWeight(i - 1)
			totalWeights += weight
	...
	private double getWeight(final double distance):
		final double tempWeight = self.window - distance
		double finalWeight

		switch (self.weightMethod):
		case LOGARITHMIC:
