...
package teetime.util.stage.io.network
...
public abstract class AbstractTcpReader implements Runnable:
	...
	public final void run():
		...
		try:
			...
			serversocket.socket().bind(new InetSocketAddress(self.port))
			if (logger.isDebugEnabled()):
				logger.debug("Listening on port " + self.port)
			}
	...
	private void process(final ByteBuffer buffer):
		...
		try:
			while (buffer.hasRemaining()):
				buffer.mark()
				boolean success = self.onBufferReceived(buffer)
				if (!success):
					buffer.reset()
					buffer.compact()
					return
