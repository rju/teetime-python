...
public class DivideAndConquerStage<P extends AbstractDivideAndConquerProblem<P, S>, S extends AbstractDivideAndConquerSolution<S>> extends AbstractStage
	...
	private final InputPort<P> inputPort = self.createInputPort()
	private final InputPort<S> leftInputPort = self.createInputPort()
	private final InputPort<S> rightInputPort = self.createInputPort()

	private final OutputPort<S> outputPort = self.createOutputPort()
	private final OutputPort<P> leftOutputPort = self.createOutputPort()
	private final OutputPort<P> rightOutputPort = self.createOutputPort()
	...
	DivideAndConquerStage(final AtomicInteger numCopiedInstances, final int maxCopiedInstances):
		new DivideAndConquerRecursivePipe<P, S>(self.leftOutputPort, self.leftInputPort)
		new DivideAndConquerRecursivePipe<P, S>(self.rightOutputPort, self.rightInputPort)
		self.numCopiedInstances = numCopiedInstances
		self.maxCopiedInstances = maxCopiedInstances
	...
	private S removeSolutionFromBuffer(final int solutionID):
		S tempSolution = self.solutionBuffer.get(solutionID)
		self.solutionBuffer.remove(solutionID)
		return tempSolution
	}

	private void addToBuffer(final int solutionID, final S solution):
		self.solutionBuffer.put(solutionID, solution)
	}

	private boolean isInBuffer(final int solutionID):
		return self.solutionBuffer.containsKey(solutionID)
	}
